...
def set_contact():
    ...
    conn = oci.connect('scott/tiger@127.0.0.1:1521/xe')
    # sql = "insert into contact Values ('{0}', '{1}', '{2}', '{3}')".format(name, phone_name, email, addr)
    sql = "insert into contact Values (:0, :1, :2, :3)"
    cursor = conn.cursor()
    cursor.execute(sql, [name,phone_name,email,addr])
    cursor.close()
    conn.commit()  # 잊지 말자
    conn.close()
...
def delete_contact(name):
    ...
    # 해당이름과 같은 요소를 찾아서 삭제
    conn = oci.connect('scott/tiger@127.0.0.1:1521/xe')
    sql = "delete from contact where name = :0"
    cursor = conn.cursor()
